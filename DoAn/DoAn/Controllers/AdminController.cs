...
namespace DoAn.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            // --- SỬA LOGIC KIỂM TRA QUYỀN Ở ĐÂY ---
            // 1. Lấy vai trò ra một cách an toàn (tránh lỗi null)
            int vaiTro = 0;
            if (Session["VaiTro"] != null)
            {
                // Dùng Convert.ToInt32 an toàn hơn (int)Session[...]
                vaiTro = Convert.ToInt32(Session["VaiTro"]);
            }

            // 2. Quy ước: 1 là Admin (Quản trị), còn lại là Nhân viên
            bool isAdmin = (vaiTro == 1);

            // Gửi cờ này sang View để ẩn/hiện giao diện
            ViewBag.IsAdmin = isAdmin;

            // ---------------------------------------

            // Các dữ liệu chung (Ai cũng thấy)
            ViewBag.SoDonHangMoi = db.tblHoaDon.Count(d => d.TinhTrang == 1);

            // Dữ liệu chỉ dành cho Admin
            if (isAdmin)
            {
                ...
                var completedOrders = db.tblHoaDon
                    .Where(d => d.DaThanhToan == true && d.NgayLap.HasValue && d.NgayLap.Value.Year == currentYear)
                    .ToList();
                ...
                ViewBag.ChartData = string.Join(",", revenueByMonth);
